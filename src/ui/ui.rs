...
use crate::crafter::app::App;

pub fn ui<B: Backend>(f: &mut Frame<B>, app: &App, message: &Arc<Mutex<String>>) {
    let size = f.size();
    ...
    match app.index {
        0 => ui_home(f, app, message, chunks[1]),
...
// Home Tab
pub fn ui_home<B>(f: &mut Frame<B>, app: &App, message: &Arc<Mutex<String>>, area: Rect)
where
    B: Backend,
{
    let chunks = Layout::default()
    ...
    draw_status(f, app, message, chunks[1]);
}
...
fn draw_status<B>(f: &mut Frame<B>, app: &App, message: &Arc<Mutex<String>>, area: Rect)
where
    B: Backend,
{
    // Status
    ...
    f.render_widget(Paragraph::new(text), status[4]);
}
